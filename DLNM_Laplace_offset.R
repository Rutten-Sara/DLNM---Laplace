DLNM_Laplace <- function(model,
  ...
  mu <- function(xi) exp(as.numeric(X %*% xi)+log(offset))
  W <- function(xi) Matrix::Diagonal(x = exp(as.numeric(X %*% xi)+log(offset)))
  s <- function(gam) exp(gam)
  
  log_pxi <- function(xi, Qv) {
    post <- sum(y * as.numeric(X %*% xi+log(offset)) -s(X %*% xi+log(offset))) - .5 * Matrix::t(xi) %*% Qv %*% xi
